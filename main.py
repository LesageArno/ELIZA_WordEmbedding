...
import eliza
import Initializer.WE_vectorial_comparison_class as WEVec
import os
import inspect
...
class App(ctk.CTk):
    def __init__(self):
        ...
        ## Détection et listage automatique des distance ##
        self.distmenue = ctk.CTkOptionMenu(self,
                                           values = [i.removesuffix("Distance") for i in dir(WEVec.VecorialComparison) if i.endswith("Distance") and i.removesuffix("Distance") not in ["init","nearest","compute"]],
                                           anchor = "w", font = ctk.CTkFont("Arial"))
        self.distmenue.set("cosine")
        ...
        self.seuil = ctk.CTkEntry(self, placeholder_text = "Threshold", font = ctk.CTkFont("Arial"))
        
        ## Définition de la correction ##
        self.correction = ctk.CTkEntry(self, placeholder_text = "Correction (None)", font = ctk.CTkFont("Arial"))
        
        ## Utilisation de PWE ou WE, PWE par défaut ##
        self.weighted = ctk.CTkSwitch(self, text = "Weighted", font = ctk.CTkFont("Arial"))
        ...
        ## Quitter ELIZA ##
        self.final = ctk.CTkButton(self, text = "Quit", command = self.final_call_back,
    ...
    def eliza_call_back(self):
        ...
        eliza.main(WEdict = self.optionmenue.get(), SEUIL = seuil, WEIGHTED = bool(self.weighted.get()),
                   ...
                   DISTANCE_MODEL = self.distmenue.get())
        '''try:
            seuil = float(self.seuil.get())
            eliza.main(WEdict = self.optionmenue.get(), SEUIL = seuil, WEIGHTED = bool(self.weighted.get()),
                   LOG = bool(self.log.get()), MATCHLOGS = bool(self.matchlog.get()),
